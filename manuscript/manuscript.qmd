...
author-note:
  ...
  disclosures:
    study-registration: ~
    data-sharing: The code to recreate this manuscript, the source code for *shinybeez*, and the most current URLs to access *shinybeez* is located at [https://github.com/brentkaplan/shinybeez](https://github.com/brentkaplan/shinybeez). The URL (at the time of publication) to access the *shinybeez* app is [https://brentkaplan.shinyapps.io/shinybeez/](https://brentkaplan.shinyapps.io/shinybeez/).
    related-report: ~
    conflict-of-interest: The authors have no conflict of interest to declare.
    financial-support: ~
    gratitude: ~
    authorship-agreements: ~
abstract: This paper introduces *shinybeez*, a free and open-source web application designed to streamline behavioral economic analyses of demand and discounting data. Behavioral economic methods have grown in popularity and while quantitative modeling of these phenomena has led to translational successes in clinical practice and policy, complex analyses has remained a barrier for many researchers and practitioners. *shinybeez* addresses this gap by providing an intuitive interface for conducting descriptive and inferential analyses without requiring programming expertise. The app integrates features previously scattered across multiple tools, allowing users to upload data, calculate empirical measures, identify systematic datasets, fit nonlinear models, and visualize results - all within a single platform. *shinybeez* supports various analysis types for demand data and can score multiple discounting tasks, including the 27-Item Monetary Choice Questionnaire. Built on R Shiny and leveraging existing R packages, the app ensures consistency with underlying analytical methods while remaining flexible for future enhancements. Key advantages of *shinybeez* include its accessibility through web browsers or local installation, ability to handle large datasets, and customizable data visualization options. By consolidating behavioral economic tools into a user-friendly interface, *shinybeez* aims to broaden the reach of these analytical techniques and facilitate their application in addressing societal issues.
...
One of the major advantages of demand and discounting is the robust
modeling of these phenomena, which has evolved over the past several decades.
...
installed. Moreover, the programming language used in these open-source tools
often requires some degree of user sophistication to command line communicate
with the packages or interfaces and execute their desired analyses or data
engagement.
...
and *beezdiscounting* [@kaplanBeezdiscountingBehavioralEconomic2023], which
...
R Shiny [@changShinyWebApplication2024]. Thus, we have aptly named
this web app *shinybeez* for its reliance on R Shiny to interface with the
*beezdemand* and *beezdiscounting* R packages (@fig-beezfam). We will first
describe some
...
where
the source code lives (hopefully indefinitely). This Github repsitory
...
Shinyapps.io is a self-service
platform that allows seamless deployment of Shiny apps.
...
in the respository under the folder manuscript/. Here, you will find the Quarto
[@Allaire_Quarto_2024] document along with any additional assets required
to generate the manuscript (e.g., renv package, images).
We would also like to acknowledge the developers of the apaquarto extension
...
template files that users will need to ensure their uploaded spreadsheets
(in either .csv or .tsv [comma or tab separated format]) are in accordance
with one of these templates. *shinybeez* can ingest either "wide form"
...
on whether the user has demand or discounting data. Users may also click
...
whether Q~0~ should be constrained to a specific value, select whether
...
top of @fig-plotsettings). Empirical
...
$\alpha$ (including standard error and confidence intervals), R^2, model
...
[see @gilroyExactSolutionUnit2019]. The last column of the table provides
...
will fit curves as just described except *within* each group. There is no
...
whether to (pseudo)log transform either the x or y axes (@fig-plotsettings).
...
![Demand Plot Settings](assets/demand-plot-settings.png){#fig-plotsettings apa-note="Top box shows the exportable table of empirical demand measures. Bottom box shows visualization of the Fit to Group (mean) with group specified and the customizable settings for demand plots." width="49%"}

When Fit to Group (pooled) analysis type is selected, all data points
will display with the best-fit line (@fig-pooled). Similarly, when
a group is specified the points and lines will be colored according to the
groups (@fig-pooledgroup). When Fit to Group (mean) analysis type is specified,
the preprocessed average data points are displayed with the best fit line
(@fig-average) and when a group is specified, the points and lines will be
colored according to the groups (@fig-plotsettings). Finally, when
...
their own subplot (@fig-twostagefacet) and when there are more than
50 IDs all curves will be shown on a single plot (@fig-twostage).

![Fit to Group (Pooled)](assets/demand-fit-to-pooled-no-group.png){#fig-pooled apa-note="Top box shows the exportable table of descriptive results. Bottom box shows visualization of the Fit to Group (pooled) with no group specified." width="49%"}

![Fit to Group (Mean)](assets/demand-fit-to-mean-no-group.png){#fig-average apa-note="Top box shows the exportable table of descriptive results. Bottom box shows visualization of the Fit to Group (mean) with no group specified." width="49%"}

![Two Stage (>50 IDs)](assets/demand-two-stage-mt50.png){#fig-twostage apa-note="Top box shows the exportable and customizable systematic criteria table. Bottom box shows visualization of the Two Stage analysis option with more than 50 IDs." width="49%"}
...
Currently, *shinybeez* provides support for scoring the 27-Item Monetary
Choice Questionaire [MCQ; @kaplanAutomatingScoringDelay2016] and two variants
of the minute discounting task
...
The analyses and outputs for scoring the 27-Item MCQ are nearly identical
...
comprehensive treatment of the approaches. After calculating the small,
...
response is provided in the bottom box of the Discounting page. Again, for a
more complete description and comparisons of these methods, see the
paper by @yehDiscountingDelayedRewards2023.
...
consistency scores. The Correlations tab displays an exportable table with
...
different k values. An important reminder that choosing a different
...
### Scoring Minute Discounting Tasks
...
[@koffarnus2017user; @koffarnus2021additional]. The Results
...
*beezdemand* or *beezdiscounting* functions are directly comparable to results
derived from *shinybeez*. Second, *shinybeez* is efficient and extensible such
that additional
...
*shinybeez* to interface with new features. Given the open-source nature
of *shinybeez*, a user can also inspect how the app interfaces with the
...
(@fig-beezfam). For example, readers can expect some of the following
features to be included
in future releases including but not limited to:
...
specifications of groupings reflecting different experimental designs,
modeling cross-price elasticity data, and mixed effects models
[@kaplanApplyingMixedeffectsModeling2021a].
...
Finally, we want to ackowledge the various R packages that make *shinybeez*
possible. Among others, these packages include
...
![Fit to Group (Pooled) with Group](assets/demand-fit-to-pooled-group.png){#fig-pooledgroup apa-note="Top box shows the exportable table of empirical demand measures. Bottom box shows visualization of the Fit to Group (pooled) with group specified." width="49%"}

![Two Stage (50 or fewer IDs)](assets/demand-two-stage-lt50.png){#fig-twostagefacet apa-note="Two Stage with 50 or fewer IDs." width="49%"}
