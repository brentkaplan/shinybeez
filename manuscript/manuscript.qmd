...
price considerations for reinforcers selected for clinical interventions
...
Microsoft Excel) simply cannot accommodate. In addition, the app is
...
is readily available elsewhere (both on Github and on the
...
where the source code lives (hopefully indefinitely). This Github repository
...
in the repository under the manuscript folder. Here, you will find the Quarto
...
Edition style requirements. We also want to acknowledge the various R packages
...
We developed *shinybeez* to be intuitive for the user, no matter
...
the table as a file with a .pdf extension.
...
Breakpoint~1~ (BP~1~; the highest x value associated with a non-zero
...
accommodate zero delay), and ordinal AUC (all x values are ranked resulting in
equidistant x values). These transformations are described by
...
whether a researcher uses *shinybeez* or the underlying R packages through
the R scripting language, the results will be identical. We feel this serves
as a tremendous benefit to the scientific landscape aimed at improving
...
landscape of the field, we foresee valuable additions for future versions
...
tab in the repository or by emailing the corresponding author.
